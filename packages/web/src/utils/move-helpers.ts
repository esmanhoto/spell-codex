...
export function labelMove(m: Move, nameOf: (id: string) => string, phase?: string): string {
  ...
  switch (m.type) {
    ...
    case "MANUAL_TO_HAND":          return `To hand: ${nameOf(a.cardInstanceId)}`
    case "MANUAL_RAZE_REALM":       return `Raze realm slot ${a.slot}`
    case "MANUAL_DRAW_CARDS":       return `Draw ${a.count} card(s)`
    case "MANUAL_RETURN_TO_POOL":   return `Return to pool: ${nameOf(a.cardInstanceId)}`
    case "MANUAL_AFFECT_OPPONENT":  return `${a.action} opp. ${nameOf(a.cardInstanceId)}`
    case "MANUAL_SET_COMBAT_LEVEL":    return `Set level ${a.level}`
    case "MANUAL_SWITCH_COMBAT_SIDE":  return `Switch side: ${nameOf(a.cardInstanceId)}`
