  ...
  function formatEvent(e: GameEvent): string {
    ...
    switch (e.type) {
      case "PLAY_MODE_CHANGED":
        return `  ${p} switched mode to ${(e.mode as string).replace("_", " ")}`
      case "MANUAL_ACTIVE_PLAYER_SET":
        return `  ${p} set active player to ${playerLabel(e.activePlayer as string)}`
      case "MANUAL_DRAW_COUNT_SET":
        return `  ${p} set manual draw count to ${e.count as number}`
      case "MANUAL_MAX_HAND_SIZE_SET":
        return `  ${p} set manual hand limit to ${e.size as number}`
      case "TURN_STARTED":
