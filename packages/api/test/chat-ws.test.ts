...
describe("CHAT_MSG", () => {
  it("broadcasts to all players in the game", async () => {
    ...
    await wsHandlers.message(
      ...
      JSON.stringify({ type: "CHAT_MSG", text: "Hello!" }),
    )
  ...
  it("includes gameId, playerId, text, ts in broadcast", async () => {
    ...
    await wsHandlers.message(
      s1 as unknown as ServerWebSocket<WsData>,
      JSON.stringify({ type: "CHAT_MSG", text: "hi" }),
    )

    const msg = s1.received.find((m) => (m as { type: string }).type === "CHAT_MSG") as Record<
      string,
      unknown
    >
  ...
  it("rejects empty text", async () => {
    ...
    await wsHandlers.message(
      s1 as unknown as ServerWebSocket<WsData>,
      JSON.stringify({ type: "CHAT_MSG", text: "   " }),
    )
  ...
  it("truncates text to 500 chars", async () => {
    ...
    await wsHandlers.message(
      s1 as unknown as ServerWebSocket<WsData>,
      JSON.stringify({ type: "CHAT_MSG", text: long }),
    )
  ...
  it("rejects if socket has not joined a game", async () => {
    ...
    const err = s1.received.find((m) => (m as { type: string }).type === "ERROR") as Record<
      string,
      unknown
    >
  ...
  it("rate-limits rapid messages", async () => {
    ...
    await wsHandlers.message(
      s1 as unknown as ServerWebSocket<WsData>,
      JSON.stringify({ type: "CHAT_MSG", text: "first" }),
    )
    // Second immediately after should be rate-limited
    await wsHandlers.message(
      s1 as unknown as ServerWebSocket<WsData>,
      JSON.stringify({ type: "CHAT_MSG", text: "second" }),
    )

    const chatMsgs = s1.received.filter((m) => (m as { type: string }).type === "CHAT_MSG")
    const errors = s1.received.filter(
      (m) =>
        (m as { type: string; code?: string }).type === "ERROR" &&
        (m as { code: string }).code === "RATE_LIMITED",
    )
...
describe("CHAT_EMOTE", () => {
  it("broadcasts to all players in the game", async () => {
    ...
    await wsHandlers.message(
      s1 as unknown as ServerWebSocket<WsData>,
      JSON.stringify({ type: "CHAT_EMOTE", emote: "heart" }),
    )
  ...
  it("rejects unknown emote ids", async () => {
    ...
    await wsHandlers.message(
      s1 as unknown as ServerWebSocket<WsData>,
      JSON.stringify({ type: "CHAT_EMOTE", emote: "skull" }),
    )
  ...
  it("accepts all allowed emotes", async () => {
    ...
    for (const emote of allowed) {
      ...
      await wsHandlers.message(
        s1 as unknown as ServerWebSocket<WsData>,
        JSON.stringify({ type: "CHAT_EMOTE", emote }),
      )
  ...
  it("rate-limits rapid emotes", async () => {
    ...
    await wsHandlers.message(
      s1 as unknown as ServerWebSocket<WsData>,
      JSON.stringify({ type: "CHAT_EMOTE", emote: "thumbsup" }),
    )
