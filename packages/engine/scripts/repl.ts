...
function printState(state: GameState) {
  ...
  console.log(`\n${sep}`)
  console.log(
    `Turn ${state.currentTurn}  |  Active: ${state.activePlayer}  |  Phase: ${state.phase}`,
  )
  console.log(sep)

  for (const [pid, player] of Object.entries(state.players)) {
    const formation =
      Object.entries(player.formation.slots)
        .map(([slot, s]) => `${slot}:${s?.realm.card.name}${s?.isRazed ? "(razed)" : ""}`)
        .join(", ") || "(empty)"
    const pool =
      player.pool.map((e) => `${e.champion.card.name}(${e.champion.card.level})`).join(", ") ||
      "(none)"
    ...
    console.log(
      `  Hand (${player.hand.length}/${HAND_SIZES[state.deckSize]!.maxEnd}): ${handNames}`,
    )
...
while (!state.winner && moveCount < MAX_MOVES) {
  ...
  console.log(
    `Move ${++moveCount}: [${activePlayer}] ${move.type}` +
      ("slot" in move ? ` → slot ${(move as { slot: string }).slot}` : "") +
      ("targetRealmSlot" in move
        ? ` → realm ${(move as { targetRealmSlot: string }).targetRealmSlot}`
        : ""),
  )
